    ...
    def __init__(self, db_type=None):
        ...
        if self.db_type == "azure_sql":
            ...
            if not connection_string or connection_string.strip() == "":
                print("WARNING: AZURE_SQL_CONNECTION_STRING is empty, defaulting to SQLite")
                self.db_type = "sqlite"
...
def init_db(wrapper=None):
    ...
    with wrapper.get_connection() as conn:
        ...
        print(f"Database initialized successfully (type: {wrapper.db_type})")


# Create a default instance but don't initialize yet
db_wrapper = DatabaseWrapper()
